...
import { cxcService, pagosService } from "@/services/cxc-cxp";
...
interface AgingData {
  corriente: { total: number; cuentas: CuentaCobrar[] };
  "1_30": { total: number; cuentas: CuentaCobrar[] };
  "31_60": { total: number; cuentas: CuentaCobrar[] };
  "61_90": { total: number; cuentas: CuentaCobrar[] };
  "90_plus": { total: number; cuentas: CuentaCobrar[] };
}
...
export default function CuentasPorCobrarPage() {
  ...
  const [detalle, setDetalle] = useState<CuentaCobrar | null>(null);
  ...
  const cargarAging = async () => {
    ...
    try {
      const { data } = await cxcService.aging();
  ...
  const verDetalle = async (cuenta: CuentaCobrar) => {
    try {
      const { data } = await cxcService.getById(cuenta.id);
      setDetalle(data);
      ...
      <div className="page">
        <div className="header">
          <div className="header-left">
            ...
            <h1 className="page-title"><span>Cuentas</span> por Cobrar</h1>
          </div>
        </div>
        ...
        <div className="stats-row">
          {[
            ...
            { val: `${promedioDias} dias`, label: "Promedio dias cobro" },
          ].map((s, i) => (
        ...
        {tab === "cuentas" && (
          <>
            <div className="toolbar">
              <input className="search-input" placeholder="Buscar por numero o cliente..." value={busqueda} onChange={e => setBusqueda(e.target.value.substring(0, 100))} maxLength={100} />
            </div>

            {loading ? (
              <div className="loading">Cargando cuentas por cobrar...</div>
            ) : filtradas.length === 0 ? (
              <div className="empty-state"><p>No hay cuentas por cobrar registradas.</p></div>
            ) : (
              <div className="table-wrap">
                <table>
                  <thead>
                    <tr>
                      <th>Numero</th><th>Cliente</th><th>Monto Original</th><th>Pagado</th>
                    ...
                    {bucket.cuentas.length > 0 && (
                      <div className="table-wrap">
                        <table>
                          <thead>
                            <tr>
                              <th>Numero</th><th>Cliente</th><th>Saldo Pendiente</th><th>Vencimiento</th><th>Dias</th>
                            </tr>
                          </thead>
                          <tbody>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setDetalle(null)}>
          <div className="modal">
            ...
            <div className="detail-grid">
              <div className="detail-item"><div className="detail-label">Cliente</div><div className="detail-val">{detalle.cliente_nombre || "—"}</div></div>
